...
class DataPreprocessor:
    ...
    def get_data_augmentation(self, is_training=True):
        ...
        if is_training:
            return A.Compose(
                [
                    A.Resize(
                        height=self.data_config["image_size"][0],
    ...
    def load_dataset_info(self, dataset_path):
        ...
        logger.info(
            f"Dataset chargé: {len(image_paths)} images, {len(class_names)} classes"
        )
    ...
    def split_dataset(self, image_paths, labels):
        ...
        val_size = self.data_config["val_split"] / (
            self.data_config["val_split"] + self.data_config["test_split"]
        )
