...
class PlantDiseaseCNN(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        if self.training_config["optimizer"] == "adam":
            ...
            raise ValueError(
                f"Optimiseur {self.training_config['optimizer']} non supporté"
            )

        if self.training_config["scheduler"] == "cosine":
            scheduler = CosineAnnealingLR(
                optimizer, T_max=self.training_config["epochs"]
            )
...
class PlantDiseaseViT(pl.LightningModule):
    ...
    def __init__(self, config_path="config.yaml"):
        ...
        raise NotImplementedError(
            "ViT model temporarily disabled due to transformers compatibility issues"
        )
...
class ModelCheckpointCallback(pl.callbacks.ModelCheckpoint):
    ...
    def __init__(self, **kwargs):
        super().__init__(monitor="val_acc", mode="max", save_top_k=1, **kwargs)
...
class EarlyStoppingCallback(pl.callbacks.EarlyStopping):
    ...
    def __init__(self, patience=10, **kwargs):
        super().__init__(monitor="val_loss", patience=patience, mode="min", **kwargs)
...
def get_training_callbacks(config_path="config.yaml"):
    ...
    callbacks = [
        ModelCheckpointCallback(
            dirpath=checkpoint_path, filename="{epoch}-{val_acc:.2f}"
        ),
